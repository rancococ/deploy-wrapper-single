...
# show usage
usage=$"`basename $0` [-h|--help] [--init] [--load=xxx.tgz] [--deploy=xxx.jar]
       [-h|--help]
                       show help info.
       [--init]
                       init volume.
...
fun_deploy_jar() {
    header "deploy jar : "
    info "deploy jar : ${arg_deploy_package}"
    suffix="${arg_deploy_package##*.}"
    if [ "x${suffix}" == "xjar" ]; then
        \rm -rf "${base_dir}/volume/single/data/"
        mkdir -p "${base_dir}/volume/single/data/"
        chmod -R 777 "${base_dir}/volume/single/data/"
        \cp -rfp "${arg_deploy_package}" "${base_dir}/volume/single/data/"
